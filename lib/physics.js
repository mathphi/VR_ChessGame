const setupPhysics = function() {
    ...
    // Let's compute the center of mass of the mesh, since Ammo.js don't do that itself
    function computeMeshCenterOfMass(mesh) {
        ...
        if (Math.abs(volume_sum) > 0.0) {
            glMatrix.vec3.scale(vol_pos_sum, vol_pos_sum, 1.0 / volume_sum);
        }
        else {
            // If the mesh has no volume (e.g. a plane) -> center of mass
            glMatrix.vec3.set(vol_pos_sum, 0.0, 0.0, 0.0);
        }
    ...
    function register_object(object, mass, collision_box_mesh = null) {
        ...
        // Rotation as bullet's quaternions
        const quat = new Ammo.btQuaternion(rotation[0], rotation[1], rotation[2], rotation[3]);
        ...
        const scale = new Ammo.btVector3(scaling[0], scaling[1], scaling[2]);

        // Create object shape from mesh
        const obj_shape = createShapeFromObject(mesh, com);
        obj_shape.setLocalScaling(scale);
        ...
        const linear_velocity   = new Ammo.btVector3();
        const angular_velocity  = new Ammo.btVector3();
        function apply_force_impulse(force_vect, force_pos, dt) {
            ...
            linear_velocity.setValue(force_vect[0] * dt, force_vect[1] * dt, force_vect[2] * dt);
            angular_velocity.setValue(
        ...
        // Destroy objects not used anymore
        Ammo.destroy(orig);
        Ammo.destroy(quat);
        Ammo.destroy(scale);
        Ammo.destroy(transform);
        Ammo.destroy(rb_info);
    ...
    function unregister_object(object) {
        if (object.get_physics_body() === null)
            return;

        const body = object.get_physics_body();

        // Unregister from physics
        physicsWorld.removeRigidBody(object.get_physics_body());

        // Remove from physics objects list
        const idx = rigid_bodies.indexOf(object);
        if (idx > -1) rigid_bodies.splice(idx, 1);

        // Unbind physics body
        object.set_physics_body(null);

        // Destroy them (free the memory!)
