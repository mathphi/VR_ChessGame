        ...
        radius_min = 4.0, radius_max = 100)
{
    ...
    // Touchscreen movement variables
    let _touch_prev_x = -1;
    let _touch_prev_y = -1;
    let _touch_prev_dist = -1;
    ...
    function register_mouse() {
        function get_canvas_mouse_pos(canvas, evt) {
        ...
        canvas.addEventListener("mousemove", function(event) {
            ...
            // Get mouse position
            const pos = get_canvas_mouse_pos(canvas, event);
        ...
        canvas.addEventListener("touchend", touch_event, false);

        let prev_touch_count = 0;
        function touch_event(event) {
            // If there is no finger on the screen -> reset prev_pos
            if (event.touches.length === 0 || prev_touch_count !== event.touches.length) {
                _touch_prev_x = -1;
                _touch_prev_y = -1;
                _touch_prev_dist = -1;
            }
            // If there is one finger touching the screen
            else if (event.touches.length === 1) {
                const touch_point = event.touches.item(0);

                // Get relative position
                const pos = get_canvas_mouse_pos(canvas, touch_point);

                // Touch point coordinates relative to center of canvas
                const x = pos.x - canvas.width / 2;
                const y = pos.y - canvas.height / 2;

                if (_touch_prev_x !== -1 && _touch_prev_y !== -1) {
                    // Compute mouse displacement
                    const dx = _touch_prev_x - x;
                    const dy = _touch_prev_y - y;
            ...
            // If two fingers => pinch zoom and camera translation
            else if (event.touches.length === 2) {
                // Disable pinch/zoom,... gestures
                event.preventDefault();

                // Retrieve the two touch elements
                const touch_pt1 = event.touches.item(0);
                const touch_pt2 = event.touches.item(1);

                // Get relative position
                const pos1 = get_canvas_mouse_pos(canvas, touch_pt1);
                const pos2 = get_canvas_mouse_pos(canvas, touch_pt2);

                // Touch point coordinates relative to center of canvas
                const x1 = pos1.x - canvas.width / 2;
                const y1 = pos1.y - canvas.height / 2;
                const x2 = pos2.x - canvas.width / 2;
                const y2 = pos2.y - canvas.height / 2;

                // Compute distance between the two touch points and the average position
                ...
                if (_touch_prev_x !== -1 && _touch_prev_y !== -1) {
                    ...
                    const dx = _touch_prev_x - avg_pos.x;
                    const dy = _touch_prev_y - avg_pos.y;
                ...
                if (_touch_prev_dist !== -1) {
                    // Compute the pinch displacement
                    const delta_dist = _touch_prev_dist - dist;

                    // Change camera view radius
                    process_camera_movement(
                        0.0,
                        0.0,
                        _mouse_movement_dist * delta_dist * 10.0,
                        false
                    );
                }

                _touch_prev_x = avg_pos.x;
                _touch_prev_y = avg_pos.y;
                _touch_prev_dist = dist;
            }

            prev_touch_count = event.touches.length;
        }
    }
