        ...
        radius_min = 2.0, radius_max = 100)
{
        ...
        document.addEventListener('keydown', (event) => {
            ...
            switch (event.key) {
                ...
                case ' ':
                    ...
                    break;
                default:
                    // Ensure the key is a digit between 0 and 9
                    if(event.key.match(/^[0-9]$/g) !== null){
                        set_special_orientation(parseInt(event.key));
                    }
                    else {
                        cam_moved = false;
                    }
    ...
    function set_special_orientation(orient_id, anim_duration = 500.0) {
        if (0 <= orient_id && 9 >= orient_id) {
            anim_camera_orientation(
                numkeys_cam_settings[orient_id].zenith,
                numkeys_cam_settings[orient_id].azimuth,
                numkeys_cam_settings[orient_id].radius,
                numkeys_cam_settings[orient_id].center,
                anim_duration
            );
        }
    }
