        ...
        radius_min = 2.0, radius_max = 100)
{
    ...
    let _world_center = world_center;
    let _world_up     = world_up;
                ...
                default:
                    if(!isNaN(event.key) && +event.key>=0 && +event.key<=9){
                        anim_camera_orientation(
                            numkeys_cam_settings[parseInt(event.key)].zenith,
                            numkeys_cam_settings[parseInt(event.key)].azimuth,
                            numkeys_cam_settings[parseInt(event.key)].radius,
                            numkeys_cam_settings[parseInt(event.key)].center,
                            500.0
                        );
                    }
                    else{
    ...
    function update_camera_vectors() {
        ...
        console.log(_cam_zenith, _cam_azimuth, _cam_radius, _world_center);
    ...
    function anim_camera_orientation(zenith, azimuth, radius, coord_center, duration) {
        ...
        const init_center   = _world_center;
        ...
        // Compute delta of cam coordinates in spherical system
        const delta_zenith  = zenith - init_zenith;
        const delta_azimuth = azimuth - init_azimuth;
        const delta_radius  = radius - init_radius;
        ...
        function animate(time) {
            ...
            if (t < duration) {
                ...
                set_camera_orientation(zenith, azimuth, radius, current_center);
            }
        }
    ...
    function set_numkey_camera_view(numkey, cam_zen, cam_az, cam_rad, center) {
        numkeys_cam_settings[numkey].zenith  = cam_zen;
        numkeys_cam_settings[numkey].azimuth = cam_az;
        numkeys_cam_settings[numkey].radius  = cam_rad;
        numkeys_cam_settings[numkey].center  = center;
    }

    const numkeys_cam_settings = [
        {zenith: 60.0, azimuth: 240.0, radius: 55, center: glMatrix.vec3.fromValues(0.0, 0.0, 0.0)}, // 0
        {zenith: 45.0, azimuth: 135.0, radius: 33.0, center: glMatrix.vec3.fromValues(0.0, 0.0, 0.0)}, // 1
        {zenith: 45.0, azimuth: 180.0, radius: 33.0, center: glMatrix.vec3.fromValues(0.0, 0.0, 0.0)}, // 2
        {zenith: 45.0, azimuth: 225.0, radius: 33.0, center: glMatrix.vec3.fromValues(0.0, 0.0, 0.0)}, // 3
        {zenith: 45.0, azimuth: 90.0, radius: 33.0, center: glMatrix.vec3.fromValues(0.0, 0.0, 0.0)}, // 4
        {zenith: 0.0, azimuth: 0.0, radius: 16.0, center: glMatrix.vec3.fromValues(0.0, 0.0, 0.0)}, // 5
        {zenith: 45.0, azimuth: 270.0, radius: 33.0, center: glMatrix.vec3.fromValues(0.0, 0.0, 0.0)}, // 6
        {zenith: 45.0, azimuth: 45.0, radius: 33.0, center: glMatrix.vec3.fromValues(0.0, 0.0, 0.0)}, // 7
        {zenith: 45.0, azimuth: 0.0, radius: 33.0, center: glMatrix.vec3.fromValues(0.0, 0.0, 0.0)}, // 8
        {zenith: 45.0, azimuth: 315.0, radius: 33.0, center: glMatrix.vec3.fromValues(0.0, 0.0, 0.0)}, // 9
    ];
    ...
    return {
        ...
        set_numkey_camera_view: set_numkey_camera_view,
