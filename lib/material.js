        ...
        if (parts.length > 0) {
            switch (parts[0]) {
                case 'newmtl':
                    ...
                    current_mtl_index++;
                    materials.push(make_material(parts[1]));
                    break;
                case 'Color':
                    materials[current_mtl_index].set_color(
                    ...
                    break;
                case 'Spec_e':
                    materials[current_mtl_index].set_spec_exp(parseFloat(parts[1]));
                    break;
                case 'Spec_s':
                    materials[current_mtl_index].set_spec_strength(parseFloat(parts[1]));
                    break;
                case 'Refl_s':
                    materials[current_mtl_index].set_reflection(parseFloat(parts[1]));
                    break;
                case 'Refr_s':
                    materials[current_mtl_index].set_refr_strength(parseFloat(parts[1]));
                    break;
                case 'Refr_i':
                    materials[current_mtl_index].set_refr_index(parseFloat(parts[1]));
                    break;
            }
        }
    }
