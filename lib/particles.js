 ...
 * @param color:        Base particles color
...
const make_particle_jet = async function (
    ...
    const _particle_color = color;
    const _part_count = part_count;
    const _particle_list = new Array(_part_count);
    ...
    // Load mesh
    const _mesh = await load_mesh('objects/plane_face.obj', true, part_size);
    ...
    function init_particles() {
        ...
        for (let p = 0; p < part_count; p++) {
            ...
            const color = glMatrix.vec3.fromValues(
                _particle_color[0] + (2*Math.random() - 1) * cvf,
                _particle_color[1] + (2*Math.random() - 1) * cvf,
                _particle_color[2] + (2*Math.random() - 1) * cvf,
            );
