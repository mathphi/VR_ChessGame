const load_mesh = async function (name, load_uv = true, uniform_scale = 1.0) {
            ...
            if (parts.length > 0) {
                switch (parts[0]) {
                    ...
                    case 'v':
                        positions.push(
                            glMatrix.vec3.fromValues(
                                parseFloat(parts[1]) * uniform_scale,
                                parseFloat(parts[2]) * uniform_scale,
                                parseFloat(parts[3]) * uniform_scale
                            ));
                        break;
                    case 'vn':
                        normals.push(
                    ...
                    case 'f': {
                        ...
                        Array.prototype.push.apply(
                            vertices, load_uv ? textures[parseInt(f1[1]) - 1] : [0,0]
                        );
                        Array.prototype.push.apply(
                        ...
                        Array.prototype.push.apply(
                            vertices, load_uv ? textures[parseInt(f2[1]) - 1] : [0,0]
                        );
                        Array.prototype.push.apply(
                        ...
                        Array.prototype.push.apply(
                            vertices, load_uv ? textures[parseInt(f3[1]) - 1] : [0,0]
                        );
                        Array.prototype.push.apply(
...
const make_object = function (gl, obj, texture = null, bump_map = null, material = null) {
    ...
    // Compute tangent space for the object (only if there is a bump map)
    const tangent_space = bump_map !== null ? object_tangent_space(obj) : new Float32Array(6*obj.num_vertices);
    ...
    function draw(shader) {
        ...
        if (a_position >= 0) {
            ...
            gl.vertexAttribPointer(a_position, 3, gl.FLOAT, false, obj.vert_length * sizeofFloat, 0);
        }

        const a_texcoord = gl.getAttribLocation(shader.program, "a_texcoord");
        if (a_texcoord >= 0) {
            gl.enableVertexAttribArray(a_texcoord);
            gl.vertexAttribPointer(a_texcoord, 2, gl.FLOAT, false, obj.vert_length * sizeofFloat, 3 * sizeofFloat);
        }

        const a_normal = gl.getAttribLocation(shader.program, 'a_normal');
        if (a_normal >= 0) {
            gl.enableVertexAttribArray(a_normal);
            gl.vertexAttribPointer(a_normal, 3, gl.FLOAT, false, obj.vert_length * sizeofFloat, (obj.vert_length - 3) * sizeofFloat);
        }
        ...
        for (let i = 0 ; i <  material_indices.length ; i++) {
            ...
            gl.drawArrays(gl.TRIANGLES, offset, num_vert);
        }
    }
    ...
    function reset_physics_motion() {
        // Set all physics to null vector
