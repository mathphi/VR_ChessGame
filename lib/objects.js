    ...
    function draw(shader) {
        ...
        for (let i = 0 ; i <  material_indices.length ; i++) {
            ...
            if (bump_map !== null) {
                ...
                gl.vertexAttribPointer(att_tangent, 3, gl.FLOAT, false, 6 * sizeofFloat, offset * 3 * sizeofFloat);

                const att_bitangent = gl.getAttribLocation(shader.program, 'a_bitangent');
                gl.enableVertexAttribArray(att_bitangent);
                gl.vertexAttribPointer(att_bitangent, 3, gl.FLOAT, false, 6 * sizeofFloat, (offset * 3 + 3) * sizeofFloat);
