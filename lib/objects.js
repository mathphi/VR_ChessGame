...
const make_object = async function (gl, obj, texture = null, bump_map = null, material = null) {
    ...
    let object_material = material;

    if (material === null) {
        object_material = make_material(gl);
    }
    ...
    function activate(shader) {
        ...
        gl.vertexAttribPointer(att_pos, 3, gl.FLOAT, false, obj.vert_length * sizeofFloat, 0);

        if (obj.has_uv) {
            const att_texcoord = gl.getAttribLocation(shader.program, "a_texcoord");
            gl.enableVertexAttribArray(att_texcoord);
            gl.vertexAttribPointer(att_texcoord, 2, gl.FLOAT, false, obj.vert_length * sizeofFloat, 3 * sizeofFloat);
        }

        const att_nor = gl.getAttribLocation(shader.program, 'a_normal');
        gl.enableVertexAttribArray(att_nor);
        gl.vertexAttribPointer(att_nor, 3, gl.FLOAT, false, obj.vert_length * sizeofFloat, (obj.vert_length-3) * sizeofFloat);
        ...
        // Send object's visual properties to shader
        object_material.activate(shader);
    }
