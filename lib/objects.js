...
const make_object = async function (gl, obj, texture = null, bump_map = null, material = null) {
    ...
    function set_rotation_quat(quatertion) {
        const position = get_position();
        const scaling  = get_scaling();
        glMatrix.mat4.fromRotationTranslationScale(
            model, quatertion, position, scaling
        );
        update_physics_transform();
    }
    ...
    function set_physics_mass(mass) {
        ...
        physics_body.getCollisionShape().calculateLocalInertia(mass, inertia);
        physics_body.setMassProps(mass, inertia);
    }
    function update_physics_transform() {
        ...
        w_transform.setOrigin(new Ammo.btVector3(position[0], position[1], position[2]));
        w_transform.setRotation(new Ammo.btQuaternion(rotation[0], rotation[1], rotation[2], rotation[3]));

        // Update scale of physics body
        physics_body.getCollisionShape().setLocalScaling(new Ammo.btVector3(scaling[0], scaling[1], scaling[2]));
