            ...
            if (parts.length > 0) {
                switch (parts[0]) {
                    case 'mtllib':
                        ...
                        break;
                    case 'usemtl':
                        // Return the index of the material with the name given by usemtl
                        const material_idx = materials.findIndex(function (material) {
                            return material.name === parts[1];
                        });

                        // The list material_indices indicates the index of the first vertex
                        // which uses the corresponding material index
                        material_indices.push({
                            vert: current_vertex,
                            mtl_idx: material_idx
                        });
                        break;
                    case 'v':
                        positions.push(
...
const make_object = async function (gl, obj, texture = null, bump_map = null, material = null) {
    ...
    function draw(shader) {
        ...
        for (let i = 0 ; i <  material_indices.length ; i++) {
            ...
            //console.log(object_materials[material_indices[i].mtl_idx]);

            // Send material properties
            object_materials[material_indices[i].mtl_idx].activate(gl, shader);
            ...
            gl.vertexAttribPointer(a_position, 3, gl.FLOAT, false, obj.vert_length * sizeofFloat, offset * obj.vert_length * sizeofFloat);

            if (obj.has_uv) {
                const a_texcoord = gl.getAttribLocation(shader.program, "a_texcoord");
                gl.enableVertexAttribArray(a_texcoord);
                gl.vertexAttribPointer(a_texcoord, 2, gl.FLOAT, false, obj.vert_length * sizeofFloat, (offset * obj.vert_length + 3) * sizeofFloat);
            }

            const a_normal = gl.getAttribLocation(shader.program, 'a_normal');
            gl.enableVertexAttribArray(a_normal);
            gl.vertexAttribPointer(a_normal, 3, gl.FLOAT, false, obj.vert_length * sizeofFloat, (offset * obj.vert_length + obj.vert_length - 3) * sizeofFloat);
            ...
            if (bump_map !== null) {
                ...
                gl.vertexAttribPointer(att_tangent, 3, gl.FLOAT, false, 6 * sizeofFloat, offset * obj.vert_length * sizeofFloat);

                const att_bitangent = gl.getAttribLocation(shader.program, 'a_bitangent');
                gl.enableVertexAttribArray(att_bitangent);
                gl.vertexAttribPointer(att_bitangent, 3, gl.FLOAT, false, 6 * sizeofFloat, (offset * obj.vert_length + 3) * sizeofFloat);
