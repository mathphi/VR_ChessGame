const make_shader = async function (gl, shader_name, shaders_dir = '') {
    async function load_shaders_source(name, shaders_dir = '') {
        const base_shader_dir = (shaders_dir === '' ? '../shaders/' : shaders_dir);
        const vert_shader_file = base_shader_dir + '/' + name + '_vert.glsl';
        const frag_shader_file = base_shader_dir + '/' + name + '_frag.glsl';
