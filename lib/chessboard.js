const make_chessboard = async function(gl, phys_eng, lights_set, basedir = '.') {
    ...
    function buryCapturedPiece(capt_piece) {
        ...
        const graveyard_3d_pos = gameTo3DPosition(
            capt_piece.piece_color === 'b'
                ? glMatrix.vec2.fromValues(-3.5 - pos_offset_x, -1.0 + pos_offset_y)
                : glMatrix.vec2.fromValues(10.5 + pos_offset_x, 8.0 - pos_offset_y)
        );
    ...
    const chessboard_tex = await load_texture(gl, 'textures/chessboard_tex.png', false);
    const chessboard_bm  = await load_texture(gl, 'textures/chessboard_bm.png', false);
    ...
    const _chessboard = await make_object(gl, chessboard_mesh, chessboard_tex, null);
