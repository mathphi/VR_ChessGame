const make_chessboard = async function(gl, phys_eng, lights_set, basedir = '.') {
    ...
    async function makePiece(piece_type, piece_color) {
        const col_mesh = await _piece_cboxes[piece_type];
        const piece = await make_object(
            gl,
            _piece_meshes[piece_type],
            null, null,
            piece_color === 'b' ? _piece_black_material : _piece_white_material
        );
        phys_eng.register_object(piece, 0.0, col_mesh);
    ...
    async function setupPieces(white_pieces, black_pieces) {
        ...
        for (const [piece_type, piece_count] of Object.entries(_game_pieces_composition)) {
            for (let i = 0 ; i < piece_count ; i++) {
                // Make white piece
                white_pieces.push(await makePiece(piece_type, 'w'));
                black_pieces.push(await makePiece(piece_type, 'b'));
            }
        }
    }
        ...
        for (const move of possible_moves) {
            ...
            if (move_success) {
                ...
                if (typeof (_on_game_event_callback) === 'function') {
                    //_on_game_event_callback();
                }
            }

            if (_ai_enabled) {
                setTimeout(processAIMove, _ai_action_delay);
            }
    ...
    function checkMoveFlag(move){
        ...
        if (move.flags.includes('n') || move.flags.includes('b')) {
            ...
            move_success = processPieceMove(move);
        }
           ...
           // En passant
            move_success = processPieceEnPassant(move);
            }
        else if (move.flags.includes('k')) {
            // Kingside castling
            move_success = processPieceCastling(move);
        }
    ...
    function processAIMove(){
        ...
        //TODO: fix the bug when you click a ennemy piece while AI turn which let the piece flying

        const move = _ai_inst.findBestMove(_chess_js_inst);
        if(_chess_js_inst.turn()==='w'){
            for(let piece of _pieces_white){
                if(piece.piece_pos !== null && glMatrix.vec2.equals(letterToGamePosition(move.from),piece.piece_pos)){
                    _selected_piece = piece;
                }
            }
        }
        else{
            for(let piece of _pieces_black){
                if(piece.piece_pos !== null && glMatrix.vec2.equals(letterToGamePosition(move.from),piece.piece_pos)){
                    _selected_piece = piece;
                }
            }
    ...
    function processPieceMove(move_data) {
        // Convert positions
        ...
        function move_done() {
            ...
            if (move_data.flags.includes('p')) {
                ...
                processPiecePromotion(selected_piece);
            }
        }
    }
    ...
    function processPieceEnPassant(move_data) {
        const from_pos  = letterToGamePosition(move_data.from);
    ...
    function processPieceCastling(move_data) {
        const from_pos  = letterToGamePosition(move_data.from);
        const to_pos    = letterToGamePosition(move_data.to);
        ...
        if (move_result !== null) {
            ...
            selected_piece.anim_move_above_pos(target_3d_pos, 3.0, 750.0);
    ...
    function processPiecePromotion(piece_object) {
        ...
        // Prepare the new piece (makePiece is async!)
        const piece_promise = makePiece('q', piece_object.piece_color);
        
        function promote() {
            // Callback when piece is fully loaded
            piece_promise.then(function (new_piece) {
                ...
                new_piece.anim_to_position(init_pos, 500.0);
            });
        }
    }
