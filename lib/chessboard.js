const make_chessboard = async function(gl, phys_eng, obj_basedir = 'objects/') {
    ...
    const _checkerboard_offset  = glMatrix.vec3.fromValues(5.95, 1.3, -5.95);
    ...
    async function setup_pieces(white_pieces, black_pieces) {
        const piece_meshes = {
            p: load_obj(obj_basedir + '/pieces/pawn.obj', false),
            b: load_obj(obj_basedir + '/pieces/bishop.obj', false),
            r: load_obj(obj_basedir + '/pieces/rook.obj', false),
            n: load_obj(obj_basedir + '/pieces/knight.obj', false),
            q: load_obj(obj_basedir + '/pieces/queen.obj', false),
            k: load_obj(obj_basedir + '/pieces/king.obj', false)
        }
        const piece_cboxes = {
            p: load_obj(obj_basedir + '/collision_boxes/c_pawn.obj', false),
            b: load_obj(obj_basedir + '/collision_boxes/c_bishop.obj', false),
            r: load_obj(obj_basedir + '/collision_boxes/c_rook.obj', false),
            n: load_obj(obj_basedir + '/collision_boxes/c_knight.obj', false),
            q: load_obj(obj_basedir + '/collision_boxes/c_queen.obj', false),
            k: load_obj(obj_basedir + '/collision_boxes/c_king.obj', false)
        }
        ...
        for (const [piece_type, piece_count] of Object.entries(_game_pieces_composition)) {
            for (let i = 0 ; i < piece_count ; i++) {
                ...
                phys_eng.register_object(wp, 1.0, col_mesh);
                ...
                phys_eng.register_object(bp, 1.0, col_mesh);
    ...
    function on_chess_picked(picked_object, picked_position) {
        const game_pos = squareFrom3DPosition(picked_position);

        if (game_pos !== null) {
            console.log("Chessboard clicked at: " + gameToLetterPosition(game_pos));
        }
    }
    ...
    return {
        ...
        on_chess_picked: on_chess_picked
    }
};