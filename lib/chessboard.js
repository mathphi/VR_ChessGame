...
const make_chessboard = async function(gl, phys_eng, lights_set, basedir = '.') {
    ...
    function force_physics(force) {
        ...
        _chessboard.get_physics_body().set_mass(10.0);
    }
    ...
    async function setupPieces() {
        ...
        for (const [piece_type, piece_count] of Object.entries(_game_pieces_composition)) {
            for (let i = 0 ; i < piece_count ; i++) {
                ...
                // Make white piece
                _pieces_white[list_idx] = await makePiece(piece_type, 'w');
                _pieces_black[list_idx] = await makePiece(piece_type, 'b');
    ...
    function setChessPiecesPosition() {
        const chess_board = _chess_js_inst.board();
        ...
        for (let k = 0 ; k < 8 ; k++) {
            for (let l = 0 ; l < 8 ; l++) {
                ...
                const piece_list = (square.color === 'b') ? _pieces_white : _pieces_black;
    ...
    function reset_game() {
        ...
        // setup pieces (asynchronous function)
        setupPieces().then(function () {
            // Place the pieces at the right position
            setChessPiecesPosition();
            updatePieces3DPosition(2000.0);
        });

        // Reset status variables and flags
        _selected_piece = null;
        _turn_running = false;
        _graveyard_count = {w: 0, b: 0};
        _possible_moves = [];
    }
