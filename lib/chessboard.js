...
const make_chessboard = async function(gl, camera, phys_eng, lights_set, particle_engine, basedir = '.') {
    ...
    let _piece_white_material = make_material(
        'white_piece',
        glMatrix.vec3.fromValues(1.0, 1.0, 1.0),
        100.0, 1.0,
        0.8
    );
    let _piece_black_material = make_material(
        'black_piece',
    ...
    function register_on_ai_thinking_callback(callback) {
        _on_ai_thinking_callback = callback;
    }
    ...
    function enable_ai(enabled, depth = 0) {
        ...
        if (get_turn() === 'b' && !_turn_running && !_ai_thinking) {
            initializeAIMove();
        }
    }
    ...
    function set_pieces_material(white_mtl, black_mtl) {
        for (const piece of _pieces_white) {
            piece.materials['white_piece'] = white_mtl;
        }
        for (const piece of _pieces_black) {
            piece.materials['black_piece'] = black_mtl;
        }

        // Save the material for future pieces creation
        _piece_white_material = white_mtl;
        _piece_black_material = black_mtl;
    }
    ...
    function resetPromoted() {
        for (const piece of _pieces_white.concat(_pieces_black)) {
            if (piece.promoted === true) {
                // Regenerate a simple pawn
                // Prepare the new piece
                const new_piece = makePiece('p', piece.piece_color);

                // Place the piece at right position (game pos and 3D pos)
                new_piece.piece_pos = piece.piece_pos;
                new_piece.set_position(piece.get_position());
                new_piece.set_rotation_quat(piece.get_rotation());
                new_piece.set_scaling(piece.get_scaling());

                // Replace the object in the pieces list
                const pieces_list = piece.piece_color === 'w' ? _pieces_white : _pieces_black;
                const piece_idx = pieces_list.indexOf(piece);
                pieces_list[piece_idx] = new_piece;

                // Remove the previous object from physics
                phys_eng.unregister_object(piece);
            }
        }
    }

    function placePromoteFirstFreePawn(color, game_pos) {
        const pieces_list = (color === 'w') ? _pieces_white : _pieces_black;

        for (const piece of pieces_list) {
            // Look for the first pawn without game position
            if (piece.piece_type === 'p' && piece.piece_pos === null) {
                // We found an unused pawn -> promote to queen

                // Prepare the new piece
                const new_piece = makePiece('q', color);

                // Place the piece at right position (game pos and 3D pos)
                new_piece.piece_pos = game_pos;
    ...
    function setChessPiecesPosition() {
        ...
        for (let k = 0 ; k < 8 ; k++) {
            for (let l = 0 ; l < 8 ; l++) {
                ...
                // If there is no piece in stock for this position
                if (used_piece === null) {
                    placePromoteFirstFreePawn(square.color, game_pos);
                }
            }
        }
    ...
    function processBoardClick(click_pos) {
        ...
        for (const move of possible_moves) {
            ...
            if (_ai_enabled) {
                setTimeout(initializeAIMove, Math.random() * _ai_action_delay);
            }
    ...
    function initializeAIMove() {
        ...
        // Start AI best move finding in the dedicated worker
        findBestMoveAI();
    }

    function processAIMove(move) {
        // In case of checkmate, there is no best move
        // We don't accept the move from AI Worker if AI has been disabled in between
        if (move === null || !_ai_enabled || _chess_js_inst.turn() !== 'b') {
            return;
        }
    ...
    function undo_game() {
        ...
        if (_chess_js_inst.history().length === 0) {
            return;
        }
    ...
    function get_moves_history() {
        return _chess_js_inst.history({verbose: true});
    }

    function get_board_str() {
        return _chess_js_inst.pgn();
    }

    function load_board_str(pgn_str) {
        const pgn_valid = _chess_js_inst.load_pgn(pgn_str);

        // Ensure the PGN string is valid
        if (!pgn_valid)
            return false;
    ...
    // AI Worker and settings
    const _ai_worker = new Worker(basedir + '/lib/chess_ai.js');
    let _ai_enabled  = false;
    let _ai_level    = 0;
    let _ai_thinking = false;

    // Process AI move when worker answers
    _ai_worker.onmessage = function(e) {
        // Send AI NOT thinking event
        if (typeof(_on_ai_thinking_callback) === 'function') {
            _on_ai_thinking_callback(false);
            _ai_thinking = false;
        }

        processAIMove(e.data.best_move);
    }
    function findBestMoveAI() {
        // Send game state and AI level to worker to start finding the best move
        _ai_worker.postMessage({game_state: _chess_js_inst.pgn(), ai_depth: _ai_level});

        // Send AI thinking event
        if (typeof(_on_ai_thinking_callback) === 'function') {
            _on_ai_thinking_callback(true);
            _ai_thinking = true;
        }
    }
    ...
    let _on_ai_thinking_callback = null;
    ...
    return {
        ...
        register_on_ai_thinking_callback: register_on_ai_thinking_callback,
        get_turn: get_turn,
        enable_ai: enable_ai,
        ...
        undo_game: undo_game,
        get_moves_history: get_moves_history,
        get_board_pgn: get_board_str,
        load_board_pgn: load_board_str,
        playEndgameSound: playEndgameSound,
        set_pieces_material: set_pieces_material,
    }
};